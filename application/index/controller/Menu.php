...
class Menu extends Controller
{
    ...
    public function shangpin()
    {
        ...
        if ($request->isAjax()) {
            ...
            $fenlei = $request->get('fenlei');//添加部分
            if(!empty($fenlei) ){
                $data = ['fenlei'=>['=',$fenlei]];
            }
            $pagesize = $request->get('limit') ? $request->get('limit') : 10;//前端直接传过来的数据
    ...
    public function fenlei()
    {
        ...
        if ($request->isAjax()) {
            ...
            $page = $request->get('page') ? $request->get('page') : 1;//前端直接传过来的数据
            $menu = new MenuModel();//调用model的getMenusCount和getMenus方法
            $res['count'] = $menu->fenlei($data);
            $res['data'] = $menu->getfenlei($data, $page, $pagesize);
            $res['code'] = 0;
            $res['msg'] = '';
            echo json_encode($res);
            exit;
    ...
    public function tianjiafenlei()
    {
        $request=Request::instance();
        if ($request->isAjax()) {
            $menu_id = $request->post('fenlei');
            $tmp=Db::table('fenlei')->where('fenlei',$menu_id)->find();
            if($tmp){
                return 0;
            }
            else {
                $data = ['fenlei' => $menu_id];
                $req = Db::table('fenlei')->insert($data);
                if ($req) {
                    return 1;
                } else return 0;
            }
    ...
    public function deletefenlei()
    {
        $request=Request::instance();
        if($request->isAjax())
        {
            $menu_id = $request->post('fenlei');
            $tmp=Db::table('think_shangpin')->where('fenlei',$menu_id)->find();
            if($tmp){
                return 2;
            }
            else{
            $menu = new MenuModel();
            $result = $menu->deletefenlei($menu_id);
            if($result) {
                return 1;
            } else {
                return 0;
    ...
    public function zhuangtai()
    {
        ...
        if($request->isAjax())
        {
            ...
            $result=Db::table('dingdan')->where('dingdan_id', $menu_id)->update(['zhuangtai' => "已完成",'wanchengshijian'=> date('Y-m-d H:i:s',time())]);
            if($result) {
                return 1;
            } else {
                return 0;
    ...
    public function tongyi()
    {
        ...
        {
            $menu_id = $request->post('shangpin_id');
            $result=Db::table('think_shangpin')->where('shangpin_id', $menu_id)->update(['yesno' => 1,'shenhe'=>'同意发布']);
    ...
    public function jujue()
    {
        ...
        {
            $menu_id = $request->post('shangpin_id');
            $result=Db::table('think_shangpin')->where('shangpin_id', $menu_id)->update(['shenhe' => "拒绝发布"]);
            if($result) {
                return 1;
            } else {
                return 0;
    ...
    public function goumai()
    {
        ...
        if($request->isAjax())
        {
            ...
            ,'shangpin_name'=>$admin_info['shangpin_name'],'url'=>$admin_info['url'],'value'=>$admin_info['value'],
                ...
                'zhuangtai'=>'等待卖家同意','goumaishijian'=>date('Y-m-d H:i:s',time())
            ];
